...
pub(crate) mod internal {
    ...
    pub fn transform_json(
        ...
        let manifest = match transform_json_manifest(source) {
    ...
    #[cfg(feature = "yaml")]
    pub fn transform_yaml(
        name: Ident,
        map_name: Ident,
        variant_name: Option<String>,
        source: &str,
    ) -> proc_macro2::TokenStream {
        let manifest = match transform_yaml_manifest(source) {
            Ok(manifest) => manifest,
            Err(e) => return anyhow_error_to_compile_error(e),
        };
    ...
    #[cfg(feature = "json")]
    pub(crate) fn transform_json_manifest(source: &str) -> anyhow::Result<Manifest> {
        Ok(serde_json::from_str(source)?)
    }

    #[cfg(feature = "yaml")]
    pub(crate) fn transform_yaml_manifest(source: &str) -> anyhow::Result<Manifest> {
