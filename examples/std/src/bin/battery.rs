...
use embedded_batteries_async::{charger::{MilliAmps, MilliVolts}, smart_battery::{BatteryStatusFields, CapacityModeValue, ManufactureDate}};
use log::*;
...
mod espi_service {
   ...
   pub struct Service {
        ...
        signal: Signal<NoopRawMutex, battery_service::ESpiMessage>
    }

    impl Service {
        pub fn new() -> Self {
            Service { endpoint: comms::Endpoint::uninit(EndpointID::External(External::Host)), signal: Signal::new() }
        }
    }
    ...
    impl comms::MailboxDelegate for Service {
        fn receive(&self, message: &comms::Message) {
            ...
            if let Some(msg) = message.data.get::<battery_service::ESpiMessage>() {
                match msg {
                    battery_service::ESpiMessage::BatCycleCount(cycles) => info!("Bat cycles: {}", cycles),
                    _ => todo!()
                }
            }
        }
    ...
    pub async fn init() {
        ...
        comms::register_endpoint(espi_service, &espi_service.endpoint).await.unwrap();
    }
}
...
impl<I2c: embedded_hal_async::i2c::I2c> embedded_batteries_async::smart_battery::SmartBattery for MockFuelGauge<I2c> {
    ...
    async fn remaining_time_alarm(&mut self, time: embedded_batteries_async::smart_battery::Minutes) -> Result<embedded_batteries_async::smart_battery::Minutes, Self::Error> {
        Ok(time)
    }
    ...
    async fn average_current(&mut self) -> Result<embedded_batteries_async::smart_battery::MilliAmpsSigned, Self::Error> {
        Ok(0)
    }
    ...
    async fn relative_state_of_charge(&mut self) -> Result<embedded_batteries_async::smart_battery::Percent, Self::Error> {
        Ok(0)
    }

    async fn absolute_state_of_charge(&mut self) -> Result<embedded_batteries_async::smart_battery::Percent, Self::Error> {
        Ok(0)
    }

    async fn remaining_capacity(&mut self) -> Result<embedded_batteries_async::smart_battery::CapacityModeValue, Self::Error> {
        Ok(CapacityModeValue::MilliAmpUnsigned(0))
    }

    async fn full_charge_capacity(&mut self) -> Result<embedded_batteries_async::smart_battery::CapacityModeValue, Self::Error> {
        Ok(CapacityModeValue::MilliAmpUnsigned(0))
    }
    ...
    async fn battery_status(&mut self) -> Result<embedded_batteries_async::smart_battery::BatteryStatusFields, Self::Error> {
    ...
    async fn design_capacity(&mut self) -> Result<embedded_batteries_async::smart_battery::CapacityModeValue, Self::Error> {
        Ok(CapacityModeValue::MilliAmpUnsigned(0))
    }
    ...
    async fn manufacture_date(&mut self) -> Result<embedded_batteries_async::smart_battery::ManufactureDate, Self::Error> {
...
battery_service::create_battery_service!(MockCharger, embedded_hal_mock::common::Generic<Transaction>, MockFuelGauge, embedded_hal_mock::common::Generic<Transaction>);
...
async fn battery_timer_callback() {
    ...
    loop {
        ...
        s.broadcast_dynamic_acpi_msgs(&[battery_service::ESpiMessage::BatCycleCount(cycle_count)]).await;
        cycle_count += 1;
