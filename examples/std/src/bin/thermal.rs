...
async fn init_sensor(spawner: Spawner) {
    ...
    spawn_sensor_tasks(spawner, sensor, mock_pin).await;
...
async fn init_fan(spawner: Spawner) {
    ...
    spawn_fan_tasks(spawner, fan).await;
}
...
thermal_service::impl_sensor_tasks!(MockSensor, MockAlertPin);
thermal_service::impl_fan_tasks!(MockFan);
