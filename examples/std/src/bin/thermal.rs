...
async fn host() {
    ...
    host.tp
        .send(thermal_id, &mptf::Request::SetVar(mptf::uid::FAN_ON_TEMP, 3131))
        .await
        .unwrap();
    Timer::after_millis(100).await;
    ...
    host.tp
        .send(thermal_id, &mptf::Request::SetVar(mptf::uid::FAN_RAMP_TEMP, 3231))
        .await
        .unwrap();
    Timer::after_millis(100).await;
    ...
    host.tp
        .send(thermal_id, &mptf::Request::SetVar(mptf::uid::FAN_MAX_TEMP, 3531))
        .await
        .unwrap();
    Timer::after_millis(100).await;
    ...
    loop {
        ...
        host.tp
            .send(thermal_id, &mptf::Request::GetVar(mptf::uid::FAN_CURRENT_RPM))
            .await
            .unwrap();
    }
}
...
async fn handle_mptf_requests() {
    loop {
        let response: mptf::Response = match ts::wait_mptf_request().await {
            ...
            mptf::Request::GetVar(mptf::uid::FAN_CURRENT_RPM) => {
                match ts::execute_fan_request(ts::fan::DeviceId(0), ts::fan::Request::GetRpm).await {
                    Ok(ts::fan::ResponseData::Rpm(rpm)) => {
                        Ok(mptf::ResponseData::GetVar(mptf::uid::FAN_CURRENT_RPM, rpm as u32))
                    }
                    _ => Err(mptf::Error::HardwareError),
                }
            }
            mptf::Request::SetVar(uid, temp_dk) => {
