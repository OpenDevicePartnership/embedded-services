        ...
        fn handle_response(&self, response: mptf::Response) {
            match response {
                ...
                Ok(mptf::ResponseData::GetVar(_instance, uid, value)) => {
                    if uid == mptf::uuid_standard::FAN_CURRENT_RPM {
...
async fn host() {
    ...
    host.tp
        .send(
            thermal_id,
            &mptf::Request::SetVar(0, mptf::uuid_standard::FAN_ON_TEMP, 3131),
        )
        .await
        .unwrap();
    Timer::after_millis(100).await;
    ...
    host.tp
        .send(
            thermal_id,
            &mptf::Request::SetVar(0, mptf::uuid_standard::FAN_RAMP_TEMP, 3231),
        )
        .await
        .unwrap();
    Timer::after_millis(100).await;
    ...
    host.tp
        .send(
            thermal_id,
            &mptf::Request::SetVar(0, mptf::uuid_standard::FAN_MAX_TEMP, 3531),
        )
        .await
        .unwrap();
    Timer::after_millis(100).await;
    ...
    loop {
        ...
        host.tp
            .send(
                thermal_id,
                &mptf::Request::GetVar(0, mptf::uuid_standard::FAN_CURRENT_RPM),
            )
            .await
            .unwrap();
    }
...
async fn handle_mptf_requests() {
    loop {
        let response: mptf::Response = match ts::wait_mptf_request().await {
            ...
            mptf::Request::GetVar(instance, mptf::uuid_standard::FAN_CURRENT_RPM) => {
                match ts::execute_fan_request(ts::fan::DeviceId(0), ts::fan::Request::GetRpm).await {
                    Ok(ts::fan::ResponseData::Rpm(rpm)) => Ok(mptf::ResponseData::GetVar(
                        instance,
                        mptf::uuid_standard::FAN_CURRENT_RPM,
                        rpm as u32,
                    )),
                    _ => Err(mptf::Error::HardwareError),
                }
            ...
            mptf::Request::SetVar(_instance, uid, temp_dk) => {
                ...
                let fan_request = match uid {
                    mptf::uuid_standard::FAN_ON_TEMP => ts::fan::Request::SetOnTemp(temp),
                    mptf::uuid_standard::FAN_RAMP_TEMP => ts::fan::Request::SetRampTemp(temp),
                    mptf::uuid_standard::FAN_MAX_TEMP => ts::fan::Request::SetMaxTemp(temp),
        ...
        ts::send_service_msg(comms::EndpointID::External(comms::External::Host), &response)
            .await
            .unwrap()
    }
}
