    ...
    impl Host {
        ...
        fn handle_response(&self, response: mptf::Response) {
            match response {
                ...
                Ok(mptf::ResponseData::GetVar(_instance, uid, value)) => {
                    if uid == mptf::uid::FAN_CURRENT_RPM {
...
async fn host() {
    ...
    host.tp
        .send(thermal_id, &mptf::Request::SetVar(0, mptf::uid::FAN_ON_TEMP, 3131))
        .await
        .unwrap();
    Timer::after_millis(100).await;
    ...
    host.tp
        .send(thermal_id, &mptf::Request::SetVar(0, mptf::uid::FAN_RAMP_TEMP, 3231))
        .await
        .unwrap();
    Timer::after_millis(100).await;
    ...
    host.tp
        .send(thermal_id, &mptf::Request::SetVar(0, mptf::uid::FAN_MAX_TEMP, 3531))
        .await
        .unwrap();
    Timer::after_millis(100).await;
    ...
    loop {
        ...
        host.tp
            .send(thermal_id, &mptf::Request::GetVar(0, mptf::uid::FAN_CURRENT_RPM))
            .await
            .unwrap();
    }
}
...
async fn init_sensor(spawner: Spawner) {
    ...
    static SENSOR: OnceLock<ts::sensor::Sensor<MockSensor>> = OnceLock::new();
...
async fn init_fan(spawner: Spawner) {
    ...
    static FAN: OnceLock<ts::fan::Fan<MockFan>> = OnceLock::new();
...
async fn handle_alerts() {
    loop {
        match ts::wait_event().await {
            ts::Event::ThresholdExceeded(sensor_id, ts::sensor::ThresholdType::WarnHigh, _) => {
                warn!("{sensor_id:?} exceeded WARN threshold");
                ts::send_service_msg(comms::EndpointID::External(comms::External::Host), &mptf::Notify::Warn).await
            }
            ts::Event::ThresholdExceeded(sensor_id, ts::sensor::ThresholdType::Prochot, _) => {
                warn!("{sensor_id:?} exceeded PROCHOT threshold");
                ts::send_service_msg(
            ...
            ts::Event::ThresholdExceeded(sensor_id, ts::sensor::ThresholdType::Critical, _) => {
                warn!("{sensor_id:?} exceeded CRITICAL threshold");
                ts::send_service_msg(
                    comms::EndpointID::External(comms::External::Host),
...
#[embassy_executor::task]
async fn handle_mptf_requests() {
    loop {
        let response: mptf::Response = match ts::wait_mptf_request().await {
            mptf::Request::GetTmp(_) => {
                match ts::execute_sensor_request(ts::sensor::DeviceId(0), ts::sensor::Request::GetTemp).await {
                    Ok(ts::sensor::ResponseData::Temp(temp)) => {
                        Ok(mptf::ResponseData::GetTmp(ts::utils::c_to_dk(temp)))
                    }
                    _ => Err(mptf::Error::HardwareError),
                }
            }
            mptf::Request::GetThrs(_) => {
                let low = ts::execute_sensor_request(
                    ts::sensor::DeviceId(0),
                    ts::sensor::Request::GetThreshold(ts::sensor::ThresholdType::WarnLow),
                )
                .await;
                let high = ts::execute_sensor_request(
                    ts::sensor::DeviceId(0),
                    ts::sensor::Request::GetThreshold(ts::sensor::ThresholdType::WarnHigh),
                )
                .await;

                match (low, high) {
                    (Ok(ts::sensor::ResponseData::Threshold(low)), Ok(ts::sensor::ResponseData::Threshold(high))) => {
                        Ok(mptf::ResponseData::GetThrs(
                            0,
                            ts::utils::c_to_dk(low),
                            ts::utils::c_to_dk(high),
                        ))
                    }
                    _ => Err(mptf::Error::HardwareError),
                }
            }
            mptf::Request::SetThrs(_, _, low, high) => {
                let low_res = ts::execute_sensor_request(
                    ts::sensor::DeviceId(0),
                    ts::sensor::Request::SetThreshold(ts::sensor::ThresholdType::WarnLow, ts::utils::dk_to_c(low)),
                )
                .await;
                let high_res = ts::execute_sensor_request(
                    ts::sensor::DeviceId(0),
                    ts::sensor::Request::SetThreshold(ts::sensor::ThresholdType::WarnHigh, ts::utils::dk_to_c(high)),
                )
                .await;
                ...
                if low_res.is_ok() && high_res.is_ok() {
                    ...
                    Err(mptf::Error::HardwareError)
                }
            }
            mptf::Request::GetVar(instance, mptf::uid::FAN_CURRENT_RPM) => {
                match ts::execute_fan_request(ts::fan::DeviceId(0), ts::fan::Request::GetRpm).await {
                    Ok(ts::fan::ResponseData::Rpm(rpm)) => Ok(mptf::ResponseData::GetVar(
                        instance,
                        mptf::uid::FAN_CURRENT_RPM,
                        rpm as u32,
                    )),
                    _ => Err(mptf::Error::HardwareError),
                }
            }
            mptf::Request::SetVar(_instance, uid, temp_dk) => {
                let temp = ts::utils::dk_to_c(temp_dk);
                let fan_request = match uid {
                    mptf::uid::FAN_ON_TEMP => ts::fan::Request::SetOnTemp(temp),
                    mptf::uid::FAN_RAMP_TEMP => ts::fan::Request::SetRampTemp(temp),
                    mptf::uid::FAN_MAX_TEMP => ts::fan::Request::SetMaxTemp(temp),
                    _ => panic!("Not supported in example demo"),
                };
                if ts::execute_fan_request(ts::fan::DeviceId(0), fan_request).await.is_ok() {
...
async fn run(spawner: Spawner) {
    ...
    spawner.must_spawn(handle_mptf_requests());
...
ts::impl_sensor_task!(mock_sensor_task, MockSensor);
ts::impl_fan_task!(mock_fan_task, MockFan);
