...
pub async fn fan_rx<T: fan_traits::Fan + fan_traits::RpmSense>(fan: &'static Fan<T>) {
...
pub async fn fan_sample<T: fan_traits::Fan + fan_traits::RpmSense>(fan: &'static Fan<T>) {
...
#[macro_export]
macro_rules! impl_fan_tasks {
    ($fan_type:ty) => {
        #[embassy_executor::task]
        async fn fan_rx_task(fan: &'static $crate::fan::Fan<$fan_type>) {
            $crate::fan::fan_rx(fan).await
        }

        #[embassy_executor::task]
        async fn fan_sample_task(fan: &'static $crate::fan::Fan<$fan_type>) {
            $crate::fan::fan_sample(fan).await
        }

        async fn spawn_fan_tasks(spawner: embassy_executor::Spawner, fan: &'static $crate::fan::Fan<$fan_type>) {
            spawner.must_spawn(fan_rx_task(fan));
            spawner.must_spawn(fan_sample_task(fan));
        }
    };
}
