...
pub enum Request {
    ...
    /// This is used when sensor exceeds the FastSamplingThreshold.
...
pub struct Sensor<T: Controller> {
    ...
    samples: Mutex<GlobalRawMutex, SampleBuf<DegreesCelsius, BUFFER_SIZE>>,
}
...
impl<T: Controller> Sensor<T> {
    ...
    pub async fn process_request(&self, request: Request) -> Response {
        match request {
            ...
            Request::GetThreshold(ThresholdType::WarnLow) => {
                Ok(ResponseData::Threshold(self.profile.lock().await.warn_low_threshold))
            }
            Request::GetThreshold(ThresholdType::WarnHigh) => {
                Ok(ResponseData::Threshold(self.profile.lock().await.warn_high_threshold))
            }
            Request::GetThreshold(ThresholdType::Prochot) => {
                Ok(ResponseData::Threshold(self.profile.lock().await.prochot_threshold))
            }
            Request::GetThreshold(ThresholdType::Critical) => {
                Ok(ResponseData::Threshold(self.profile.lock().await.crt_threshold))
            }
            ...
            Request::SetRetryLimit(limit) => {
                self.profile.lock().await.retry_limit = limit;
                Ok(ResponseData::Success)
            }
            Request::GetProfile => {
    ...
    async fn check_thresholds(&self, temp: DegreesCelsius) {
        ...
        if temp >= self.profile.lock().await.warn_high_threshold && !self.state.lock().await.is_warn_high {
            crate::send_event(crate::Event::ThresholdExceeded(
                self.device.id,
                ThresholdType::WarnHigh,
                temp,
            ))
            .await;

            self.state.lock().await.is_warn_high = true;
        } else if temp < self.profile.lock().await.warn_high_threshold && self.state.lock().await.is_warn_high {
            crate::send_event(crate::Event::ThresholdCleared(self.device.id, ThresholdType::WarnHigh)).await;
            self.state.lock().await.is_warn_high = false;
        }

        if temp <= self.profile.lock().await.warn_low_threshold && !self.state.lock().await.is_warn_low {
            crate::send_event(crate::Event::ThresholdExceeded(
                self.device.id,
                ThresholdType::WarnLow,
                temp,
            ))
            .await;

            self.state.lock().await.is_warn_low = true;
        } else if temp > self.profile.lock().await.warn_low_threshold && self.state.lock().await.is_warn_low {
            crate::send_event(crate::Event::ThresholdCleared(self.device.id, ThresholdType::WarnLow)).await;
            self.state.lock().await.is_warn_low = false;
        }

        if temp >= self.profile.lock().await.prochot_threshold && !self.state.lock().await.is_prochot {
            crate::send_event(crate::Event::ThresholdExceeded(
                self.device.id,
                ThresholdType::Prochot,
                temp,
            ))
            .await;

            self.state.lock().await.is_prochot = true;
        } else if temp < self.profile.lock().await.prochot_threshold && self.state.lock().await.is_prochot {
            crate::send_event(crate::Event::ThresholdCleared(self.device.id, ThresholdType::Prochot)).await;
            self.state.lock().await.is_prochot = false;
        }

        if temp >= self.profile.lock().await.crt_threshold && !self.state.lock().await.is_critical {
            crate::send_event(crate::Event::ThresholdExceeded(
                self.device.id,
            ...
            self.state.lock().await.is_critical = true;
        } else if temp < self.profile.lock().await.crt_threshold && self.state.lock().await.is_critical {
            crate::send_event(crate::Event::ThresholdCleared(self.device.id, ThresholdType::Critical)).await;
            self.state.lock().await.is_critical = false;
        }
    }

    async fn try_sample(&self) -> Result<f32, ()> {
        loop {
            let mut retry_limit = self.profile.lock().await.retry_limit;
            match self.controller.lock().await.temperature().await {
                Ok(temp) => return Ok(temp),
                Err(e) => {
                    retry_limit -= 1;
                    if retry_limit == 0 {
                        self.profile.lock().await.sampling_enabled = false;
                        crate::send_event(crate::Event::SensorFailure(self.device.id, Error::Hardware)).await;
                        error!("Error reading from sensor, disabling sampling: {}", e.kind());
                        return Err(());
                    }
                }
            }
    ...
    pub async fn handle_sampling(&self) {
        loop {
            ...
            if self.profile.lock().await.sampling_enabled {
                let temp = match self.try_sample().await {
                    Ok(temp) => temp,
                    _ => continue,
                };
                ...
                // TODO: Add hysteresis
                let sleep_duration = if temp >= self.profile.lock().await.fast_sampling_threshold {
                    self.profile.lock().await.fast_sample_period
                } else {
                    self.profile.lock().await.sample_period
                };
...
macro_rules! impl_sensor_task {
    ($sensor_task_name:ident, $sensor_type:ty) => {
        #[embassy_executor::task]
        pub async fn $sensor_task_name(sensor: &'static $crate::sensor::Sensor<$sensor_type>) {
            embedded_services::info!("Sensor task started!");
            let _ = embassy_futures::select::select(sensor.handle_rx(), sensor.handle_sampling()).await;
        }
    };
}
