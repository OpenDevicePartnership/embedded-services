...
pub async fn sensor_rx<T: TemperatureSensor + TemperatureThresholdSet>(sensor: &'static Sensor<T>) {
...
pub async fn sensor_sample<T: TemperatureSensor + TemperatureThresholdSet>(sensor: &'static Sensor<T>) {
...
pub async fn sensor_alert<T: TemperatureSensor + TemperatureThresholdSet, A: embedded_hal_async::digital::Wait>(
...
#[macro_export]
macro_rules! impl_sensor_tasks {
    ($sensor_type:ty, $alert_pin_type:ty) => {
        #[embassy_executor::task]
        async fn sensor_rx_task(sensor: &'static $crate::sensor::Sensor<$sensor_type>) {
            $crate::sensor::sensor_rx(sensor).await;
        }

        #[embassy_executor::task]
        async fn sensor_sample_task(sensor: &'static $crate::sensor::Sensor<$sensor_type>) {
            $crate::sensor::sensor_sample(sensor).await;
        }

        #[embassy_executor::task]
        async fn sensor_alert_task(sensor: &'static $crate::sensor::Sensor<$sensor_type>, alert_pin: $alert_pin_type) {
            $crate::sensor::sensor_alert(sensor, alert_pin).await;
        }

        async fn spawn_sensor_tasks(
            spawner: embassy_executor::Spawner,
            sensor: &'static $crate::sensor::Sensor<$sensor_type>,
            alert_pin: $alert_pin_type,
        ) {
            spawner.must_spawn(sensor_rx_task(sensor));
            spawner.must_spawn(sensor_sample_task(sensor));
            spawner.must_spawn(sensor_alert_task(sensor, alert_pin));
        }
    };
}
